...
1. 底层连接使用 UDP ，相对于 TCP 长连接，网络质量和稳定性更高。且推荐在同一个云ISP环境内跨区域，能够提供更加稳定的网络质量。
2. 在网路的中间增加一个节点，比直接连接的情况，受到的干扰较少。且就近访问，网络速度更快。
3. 全部使用开源软件，且资源私有化，保证网络环境的安全性。
...
1. 图中 **VPS 3**, **VPS 4** 请在同一家公有云平台购买（网络连接延时更优秀），操作系统选择云平台上的 `CentOS 7 X86_64` 最新映像即可。
...
| 对象 | 配置 | 资费 |
|--------|------|------|
| （深圳）VPS 3 | 1核512M, 无公网IP | 约24元/月, 或500+元/5年 |
| （香港）VPS 4 | 1核512M, 无公网IP | 约29元/月, 或500+元/5年 |
| 深圳VPS的弹性公网IP | 选择按量计费模式，带宽100M | 0.8元/G，或购买流量包 |
| 香港VPS的弹性公网IP | 选择按量计费模式，带宽100M | 1.0元/G，或购买流量包 |

费用合计预估：

1. 基础设施 **200元/年** ：按一次性购买基础设施（2台VPS）时长5年计算。
...
图中 **VPS 3** , **VPS 4** 为 `CentOS 7 X86_64` 环境，安装 wireguard 方法如下：
...
初始化 **VPS 3**, **VPS 4** 的 wireguard 配置文件，执行命令：
...
图中 **2** 为你自己的设备，比如你的笔记本，请查找相关资料安装 Wireguard 软件即可。
...
图中 **VPS 3** 为我们的一个中间节点，我们希望其作为服务器(Server)的角色。因此需要固定 UDP 监听的端口，方便客户端角色连接过来。

最终 **VPS 3** 的 `/etc/wireguard/wg0.conf` 文件配置如下：
...
[Peer]
PublicKey =  <VPS 4 的 wireguard 公钥>
AllowedIPs = 6.6.6.6/32

[Peer]
PublicKey =  <图中设备 2 的 wireguard 公钥>
...
图中 **VPS 4** 是对外的节点，由于其公网特性和安全要求，虽然是客户端角色，但是我们也固定其 UDP 监听端口。这样方便我们在云平台的安全组中开放该 UDP 端口的访问权限。

最终 **VPS 4** 的 `/etc/wireguard/wg0.conf` 文件配置如下：
...
[Peer]
PublicKey = <VPS 3 的 wireguard 公钥>
AllowedIPs = 6.6.6.0/24
Endpoint = <VPS 3 的公网IP>:12345
...
[Peer]
PublicKey = <VPS 3的 wireguard 公钥>
AllowedIPs = 6.6.6.0/24
Endpoint = <VPS 3 公网IP>:12345
PersistentKeepalive = 25
```
...
我们使用 docker-compose 启动2个 Proxy 服务：
...
64 bytes from 6.6.6.6: icmp_seq=7 ttl=64 time=43.3 ms
```
...
64 bytes from 6.6.6.1: icmp_seq=7 ttl=64 time=48.877 ms
```

在我的笔记本上 ping **VPS 4** (经过 **VPS 3**) :
...
64 bytes from 6.6.6.6: icmp_seq=7 ttl=63 time=92.859 ms
64 bytes from 6.6.6.6: icmp_seq=8 ttl=63 time=92.075 ms
64 bytes from 6.6.6.6: icmp_seq=9 ttl=63 time=90.551 ms
```
...
interface: wg0
  ...
  private key: (hidden)
  listening port: <本机 wireguard 动态UDP端口>

peer: <VPS 3 的公网IP>
  endpoint: <VPS 3 IP>:12345
